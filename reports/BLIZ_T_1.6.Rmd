...
output: 
  html_document:
    ...
    highlight: haddock
    toc: yes
    toc_depth: 3  # Show only 3 levels in the TOC
    toc_float:
      collapsed: no  # Keep the TOC expanded by default
      smooth_scroll: yes  # Enable smooth scrolling when clicking on TOC entries
---
...
Find the source code from
[here](https://github.com/andrepsilvadev/GermanGrasslandPlants/blob/dev/src/BLIZ1.6.R)
...
```{r, include=FALSE}
source("BLIZ1.6.R", local =  knitr::knit_global())
```
...
```{r, fig.dim=c(12,8), echo=FALSE, results='asis'}

for (i in seq_along(plot_list)) {
  cat(paste0("### Plot for Scenario ", scenarios[i], "\n\n"))
  print(plot_list[[i]])
}
```
...
Check this [script](https://github.com/andrepsilvadev/GermanGrasslandPlants/blob/dev/src/BLIZ1.6_SensitivityAnalysis_29Jul.R) to see how the sensitivity analysis was done.
