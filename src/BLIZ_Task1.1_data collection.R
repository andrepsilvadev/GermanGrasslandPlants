...
## ------------------------------------------------------------------------------------------------------------------------------
genus_list_ger <- read.csv("Data/genus_list_Germination.csv") #list with species and genus to estimate from
...
## ------------------------------------------------------------------------------------------------------------------------------
genus_list_den <- read.csv2("Data/genus_list_PDensity.csv") #list with species and genus to estimate from
...
genus_list <- read.csv("Data/genus_sp.csv") #list with genus to look for

sub_genus <- subset(compadre, Genus %in% genus_list$genus) #subset database for our genuses
unique(sub_genus$MatrixTreatment) #check methods

sub_genus2 <- subset(sub_genus, MatrixTreatment == "Unmanipulated" | MatrixTreatment == "Competition (C)") #filter rows with only unmanipulated values
...
genus_list_Lambda <- read.csv("Data/genus_list_Lambda.csv") #list with species and genus to estimate from
